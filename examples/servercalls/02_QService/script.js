...
mainModule.controller("mainController", function ($scope, $q, $http) {
    ...
    $scope.chainedAsyncFunc = function (executionPath) {
        ...
        asyncFunc1(executionPath).then(asyncFunc2, logExecutionResult).then(logExecutionResult, logExecutionResult);
    };
    ...
    $scope.aggregatedHTTPAsyncFunc = function (call1Success, call2Success, call3Success) {
        ...
        var promise = $q.all([
            $http.get((call1Success ? server_uri : "invalid-url.php"), getCallParams("Call1", 5)),
            $http.get((call2Success ? server_uri : "invalid-url.php"), getCallParams("Call2", 4)),
            $http.get((call3Success ? server_uri : "invalid-url.php"), getCallParams("Call3", 3))
        ]);
        $scope.aggregatedHTTPAsyncFuncResult = "Waiting for all the results...";
        promise.then(
        // Success handler
        function (httpResultArray) {
            $scope.aggregatedHTTPAsyncFuncResult = "[" + httpResultArray[0].data + ", " + httpResultArray[1].data + ", " + httpResultArray[2].data + "]";
        }, 
        // Failure handler
        function (httpResult) {
